...
def exportar_ejercicio1_excel(df, nombre_archivo='resultados_completos.xlsx'):
    """
    Exporta los resultados del Ejercicio 1 a un archivo Excel con formato específico.
    """
    try:
        if df.empty:
...
def exportar_ejercicio2_excel(df, nombre_archivo='resultados_completos2.xlsx'):
    """
    Exporta los resultados del Ejercicio 2 a un archivo Excel con formato específico.
    """
    try:
        if df.empty:
            raise ValueError("El DataFrame está vacío")
        ...
        columnas = ['Imagen', 'Tiempo (s)', 'Angulo_izq', 'Angulo_der', 'Diferencia_angular',
        ...
        column_widths = {
            ...
            'C': 15,  # Angulo_izq
            'D': 15,  # Angulo_der
            'E': 18,  # Diferencia_angular
            'F': 15,  # Perimetro_izq
            'G': 15,  # Perimetro_der
            'H': 18,  # Asimetria_perimetro
            'I': 18,  # Factor_esparcimiento
            'J': 15  # Tipo_angulo
        }
        ...
        for row in ws.iter_rows(min_row=2):
            for cell in row:
                ...
                if cell.column_letter == 'B':
                    cell.number_format = '0.00000'
                elif cell.column_letter in ['C', 'D', 'E', 'F', 'G', 'H', 'I']:
                    cell.number_format = '0.00'
