...
def graficar_resultados(df_angulos):
    ...
    if 'Angulo_izq' in df_angulos:
        ax1.plot(df_angulos['Tiempo (s)'], df_angulos['Angulo_izq'], 'b-', label='Izquierdo')
    if 'Angulo_der' in df_angulos:
        ax1.plot(df_angulos['Tiempo (s)'], df_angulos['Angulo_der'], 'r-', label='Derecho', linewidth=2)
    ...
    fig.suptitle('Análisis de Ángulos de Contacto y Parámetros Relacionados',
                fontsize=14, y=1.02)

    # Guardar con alta resolución
    fig.savefig('resultados_angulos.png', dpi=300, bbox_inches='tight')
    fig.savefig('resultados_ejercicio3.png', dpi=300, bbox_inches='tight')  # Guardar con ambos nombres
    plt.close(fig)
